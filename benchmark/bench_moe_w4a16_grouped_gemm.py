...
# Set SGL_MOE_BENCH_MODEL=gpt-oss-120b (or its gpt-oss alias) to run the real
# GPT-OSS-120B TP=4 and TP=8 prefill routing vectors from fmha-cri's W4A16 MoE
# example. Optionally set SGL_MOE_BENCH_GPT_OSS_WORKLOADS to a comma-separated
# subset, such as "prefill-l0,tp8-prefill-l35".
#
# Run:
#   python benchmark/bench_moe_w4a16_grouped_gemm.py
#   SGL_MOE_BENCH_MODEL=gpt-oss-120b python benchmark/bench_moe_w4a16_grouped_gemm.py
...
MODEL_PROFILE = os.environ.get("SGL_MOE_BENCH_MODEL", "").strip().lower()
if MODEL_PROFILE == "gpt-oss":
    MODEL_PROFILE = "gpt-oss-120b"
if MODEL_PROFILE not in ("", "gpt-oss-120b"):
    raise ValueError(
        "SGL_MOE_BENCH_MODEL must be one of: gpt-oss-120b, gpt-oss; "
        f"got {MODEL_PROFILE!r}"
    )

# Kept for users of the original one-workload benchmark switch. The unified
# model profile supersedes it and covers every production prefill workload.
RUN_GPT_OSS_TP4_L0_LEGACY = os.environ.get("SGL_MOE_BENCH_GPT_OSS_TP4_L0") == "1"
RUN_GPT_OSS_120B = MODEL_PROFILE == "gpt-oss-120b" or RUN_GPT_OSS_TP4_L0_LEGACY
...
if RUN_GPT_OSS_120B:
    profile_name = MODEL_PROFILE or "legacy tp4 l0"
    print(f"[config] GPT-OSS production profile: {profile_name}", flush=True)
...
GPT_OSS_120B_PREFILL_ROWS = {
    "prefill-l0": GPT_OSS_120B_TP4_L0_ROWS,
    "prefill-l14": (
        283,
        2509,
        0,
...
GPT_OSS_120B_PREFILL_WORKLOADS = (
    ("prefill-l0", 1472, 2880, 2880, 736),
    ("prefill-l14", 1472, 2880, 2880, 736),
    ("prefill-l35", 1472, 2880, 2880, 736),
    ("tp8-prefill-l0", 768, 2880, 2880, 384),
    ("tp8-prefill-l14", 768, 2880, 2880, 384),
    ("tp8-prefill-l35", 768, 2880, 2880, 384),
)
...
def _gpt_oss_120b_selected_workloads():
    """Return the selected GPT-OSS-120B production prefill workloads."""
    available = {workload[0] for workload in GPT_OSS_120B_PREFILL_WORKLOADS}
    configured = os.environ.get("SGL_MOE_BENCH_GPT_OSS_WORKLOADS", "").strip()
    if configured:
        selected = {name.strip() for name in configured.split(",") if name.strip()}
        unknown = selected - available
        if unknown:
            raise ValueError(
                "SGL_MOE_BENCH_GPT_OSS_WORKLOADS has unknown workload(s): "
                f"{', '.join(sorted(unknown))}; available: {', '.join(sorted(available))}"
            )
    elif RUN_GPT_OSS_TP4_L0_LEGACY and not MODEL_PROFILE:
        selected = {"prefill-l0"}
    else:
        selected = available
    return tuple(
        workload
        for workload in GPT_OSS_120B_PREFILL_WORKLOADS
        if workload[0] in selected
    )


def _run_gpt_oss_120b_benchmark():
    """Time the real ragged GPT-OSS-120B production prefill calls.
    ...
    """
    workloads = _gpt_oss_120b_selected_workloads()
    print(
        "\n[GPT-OSS-120B production routing] "
        + ", ".join(workload[0] for workload in workloads),
        flush=True,
    )
    for workload, gemm1_n, gemm1_k, gemm2_n, gemm2_k in workloads:
        rows = GPT_OSS_120B_PREFILL_ROWS[workload]
        for gemm, gemm_n, gemm_k in (
            ("gemm1", gemm1_n, gemm1_k),
            ("gemm2", gemm2_n, gemm2_k),
        ):
            name = f"gpt-oss-120b-{workload}-{gemm}"
